...
from ..domain.offline_package import OfflinePackage, build_manifest, build_package
...
class OfflinePackageService:
    ...
    def __init__(self, curriculum: CurriculumReadModel, now: Callable[[], float]) -> None:
    ...
    def list_packages(self) -> dict[str, object]:
        ...
        now_ms = self._now_ms()
        manifests = [
            build_manifest(view, now_ms=now_ms).to_dict()
    ...
    def get_package(self, lesson_id: str) -> OfflinePackage | None:
        ...
        for view in self._curriculum.published_lessons():
            if view.lesson_id == lesson_id:
                return build_package(view, now_ms=now_ms)
        return None
